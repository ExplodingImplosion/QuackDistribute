...
func _input(event):
	if event is InputEventMouseMotion:
		...
		$Head.rotation_degrees.x = clamp($Head.rotation_degrees.x - event.relative.y * sens, -90, 90)
	...
	if (Input.is_action_just_pressed("ui_cancel")):
		...
		get_tree().change_scene("res://interface/Menus/developer main menu.gd")
	
func _process(delta):
	self.myHUD.fps.text = "FPS: " + str(Engine.get_frames_per_second())
	if is_on_floor():
